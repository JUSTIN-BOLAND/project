...
<script src="<%=request.getContextPath()%>/man/js/fileinput/js/locales/zh.js" type="text/javascript"></script>
        ...
        <section class="navigation">
          <span>您当前所在的位置：</span><a  href="#" title="">充值套餐管理</a>/<i>添加充值套餐</i>
        </section>
        ...
        <div class="article">
              ...
              <div class="form-group">
                  ...
                  <input class="form-txt form-adv-txt" type="text" name="planName" value="${rechargePlan.planName}"
                         style="width: 18%;" datatype="*,uniqueRechargePlanName" sucmsg="&nbsp;" nullmsg="不能为空" errormsg="输入的套餐名称已存在">
              ...
              <div class="form-group">
                  ...
                  <input class="form-txt form-adv-txt" type="text" name="giftAmount" value="${rechargePlan.dayMaxCount}"
              ...
              <div class="form-group">
				...
				<div class="form-group">
                    ...
                    <img id="previewImg" style="z-index:1;min-width:100px;min-height:100px;max-width:500px;max-height:400px;"
                         src="<%=request.getContextPath()%>/man/images/upload.png" >
       ...
       //表单验证
	      $(function(){
		    ...
		    $(".form-add").Validform({tiptype:3,
                datatype:{
                    "uniqueRechargePlanName":function(gets,obj,curform,regxp){
                        /*参数gets是获取到的表单元素值，
                         obj为当前表单元素，
                         curform为当前验证的表单，
                         regxp为内置的一些正则表达式的引用。*/
                        var falg = false;
                        $.ajax({
                            url: '<%=request.getContextPath()%>/rechargePlan/uniqueRechargePlanName.html?rechargePlanName='+encodeURI(gets),
	      ...
	      function myClose(){
            ...
            function f_add(){
                $.ajax({
                    url: "<%=request.getContextPath()%>/rechargePlan/addRechargePlan.html",
                    type: 'POST',
                    cache: false,
                   ...
                   /* beforeSend: function(){
                    ...
                    success : function(data) {
                        ...
                        if (data.success) {
                            $("#bodyRight").empty();
                            $("#bodyRight").load("<%=request.getContextPath()%>/rechargePlan/toAddRechargePlan.html");
                            layer.msg('添加成功！', {
                                icon : 1
                            });
                        } else {
