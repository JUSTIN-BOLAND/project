...
<%-- 	             <fmt:formatDate value="${org.createTime }" pattern="yyyy-MM-dd HH:mm:ss" />  --%>
	            <t:buttonOut url="/advertise/toAddPayAdv.html">
	            <input type="button" value="编辑" data-toggle="modal" data-target="#tjdy" onclick="edit('${advertise.id}')"/>
				</t:buttonOut>
				
				<t:buttonOut url="/advertise/cancelpushadv.html">
				<c:if test="${advertise.pushstatus eq 1 }">
					<input type="button" value="取消发布"  onclick="pushadv('${advertise.id}',0)"/>
				</c:if>
				</t:buttonOut>
				<t:buttonOut url="/advertise/pushadv.html">
				<c:if test="${advertise.pushstatus eq 0 }">
					<input type="button" value="发布"  onclick="pushadv('${advertise.id}',1)"/>
				</c:if>
				</t:buttonOut>
				
				<t:buttonOut url="/advertise/deleteAdv.html">
				<input type="button" value="删除" onclick="deleteAdv('${advertise.id}')"/>
				</t:buttonOut>
