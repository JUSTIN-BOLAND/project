...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%>
        ...
        <section class="navigation">
          <span>您当前所在的位置：</span><a  href="#" title="">一卡通管理</a>/<i>修改</i>
        </section>
 ...
 <div class="form-group">
                  ...
                  <input class="form-txt form-adv-txt" type="text" name="cardNo" value="${card.cardNo}"
                         style="width: 18%;" datatype="n7,uniqueUpdateCardNo" sucmsg="&nbsp;" nullmsg="不能为空" errormsg="输入的卡号格式不对或者卡号已经存在">
                  <span class="form-yz Validform_checktip"></span>
              </div>
                  <div class="form-group">
                      <label class="form-lal"><i class="text-danger">*</i>管理密码：</label>
                      <input class="form-txt form-adv-txt iwidth" type="password" name="passwd"  plugin="passwordStrength"  value="${card.passwd}"
                  ...
                  <div class="form-group">
                      ...
                      <input class="form-txt form-adv-txt iwidth" type="password" name="repasswd" recheck="passwd" value="${card.passwd}"
 ...
 <div class="form-group">
                  ...
                  <input class="form-txt form-adv-txt" type="text" name="name" value="${card.name}"
                         style="width: 18%;" datatype="userName,uniqueUpdateCardName" sucmsg="&nbsp;" nullmsg="不能为空" errormsg="请输入的【2-6】位汉字或者姓名已经存在">
                  <span class="form-yz Validform_checktip"></span>
              </div>
...
<%-- <div class="form-group">
 ...
 <div class="form-group">
                  ...
                  <input class="form-txt form-adv-txt" type="text" name="idCardNo" value="${card.idCardNo}"
                         style="width: 18%;" datatype="idCard" sucmsg="&nbsp;" nullmsg="不能为空" errormsg="请输入的18位身份证">
                  <span class="form-yz Validform_checktip"></span>
              </div>
<div class="form-group">
       ...
       //表单验证
	      $(function(){
		    ...
		    $(".form-add").Validform({tiptype:3,
                datatype:{
                    "uniqueUpdateCardNo":function(gets,obj,curform,regxp){
                        /*参数gets是获取到的表单元素值，
                         obj为当前表单元素，
                         curform为当前验证的表单，
                         regxp为内置的一些正则表达式的引用。*/
                        var falg = false;
                        $.ajax({
                            url: '<%=request.getContextPath()%>/card/uniqueUpdateCardNo.html?cardNo='+gets+'&oldCardNo=${card.cardNo}',
                    ...
                    "uniqueUpdateCardName":function(gets,obj,curform,regxp){
                        /*参数gets是获取到的表单元素值，
                         obj为当前表单元素，
                         curform为当前验证的表单，
                         regxp为内置的一些正则表达式的引用。*/
                        var falg = false;
                        $.ajax({
                            url: '<%=request.getContextPath()%>/card/uniqueUpdateCardName.html?cardName='+gets+'&oldCardName=${card.name}',
                            context: document.body,
                            async: false,
                            timeout:5000,
                            success: function(date){
          ...
          function f_add(){
                $.ajax({
                    url: "<%=request.getContextPath()%>/card/edit.html",
                    type: 'POST',
                    cache: false,
