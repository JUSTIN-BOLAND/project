...
<style>
    ...
    .btn-bar p {
        ...
        <section class="navigation">
          <span>您当前所在的位置：</span><a  href="#" title="">设备管理</a>/<i>修改设备</i>
        </section>
 ...
 <div class="form-group">
                  ...
                  <input class="form-txt form-adv-txt" type="text" name="deviceName" value="${device.deviceName}"
                         style="width: 18%;" datatype="*,uniqueUpdateDeviceName" sucmsg="&nbsp;" nullmsg="不能为空" errormsg="输入的设备名称不对">
                  <span class="form-yz Validform_checktip"></span>
              </div>
 ...
 <div class="form-group">
                  ...
                  <input class="form-txt form-adv-txt" type="text" name="deviceCode" value="${device.deviceCode}"
                         style="width: 18%;" datatype="serial,uniqueUpdateDeviceCode" sucmsg="&nbsp;" nullmsg="不能为空" errormsg="输入的设备序列号不对">
                  <span class="form-yz Validform_checktip"></span>
              </div>
 ...
 <div class="form-group">
                  ...
                  <select   id="flag"  name="flag" multiple="multiple"  <%--disabled="disabled"--%>  class="form-sel" style="width:206.5px;height:150px;">
                      <c:forEach items="${selectPlans }" var="item" varStatus="idx">
                  ...
                  <div class="form-group">
                      ...
                      <textarea class="form-cont memowidth" name="hints" value=" ${device.hints }" rows="4"
                                style="width: 18%;" datatype="*" sucmsg="&nbsp;" nullmsg="不能为空" errormsg="请输入提示">${device.hints }</textarea>
                      <span class="form-yz Validform_checktip"></span>
                  </div>
<div class="form-group">
                        ...
                        <div class="model-box">
                            <form class="form-add"
                                ...
                                <div class="form-group" style="text-align:center;">
                                    ...
                                    <input class="form-btn"   type="button" name="" value="取消" onclick="f_closeWin()"/>
                                </div>
   ...
   <script type="text/javascript">
       ...
       function f_closeWin(){
           //alert($("select[name=flag] option").size());
       ...
       $(function(){
          ...
          <%-- $.ajax({
               url: '<%=request.getContextPath()%>/device/getSelectPlans.html?deviceId=${device.id}',
               ...
               success: function(date){
                   ...
                   for(var i = 0;i<data.length;i++){
                       html += '<option value="'+data[i].id+'">' +data[i].planName+ '</option>';
           ...
           $('#flag').click(function () {
		    ...
		    $(".form-add").Validform({tiptype:3,
                datatype:{
                    "uniqueUpdateDeviceName":function(gets,obj,curform,regxp){
                        /*参数gets是获取到的表单元素值，
                         obj为当前表单元素，
                         curform为当前验证的表单，
                         regxp为内置的一些正则表达式的引用。*/
                        var falg = false;
                        $.ajax({
                            url: '<%=request.getContextPath()%>/device/uniqueUpdateDeviceName.html?deviceName='+gets+'&oldDeviceName=${device.deviceName}',
                            context: document.body,
                            async: false,
                            timeout:5000,
                            success: function(date){
                    ...
                    "uniqueUpdateDeviceCode":function(gets,obj,curform,regxp){
                        /*参数gets是获取到的表单元素值，
                         obj为当前表单元素，
                         curform为当前验证的表单，
                         regxp为内置的一些正则表达式的引用。*/
                        var falg = false;
                        $.ajax({
                            url: '<%=request.getContextPath()%>/device/uniqueUpdateDeviceCode.html?deviceCode='+gets+'&dealerId='+$("#dealerId").val()+"&oldDeviceCode=${device.deviceCode}",
                            context: document.body,
                            async: false,
                            timeout:5000,
                            success: function(date){
          ...
          function f_add(){
              ...
              //$("#flag").removeAttr("disabled");
                ...
                $.ajax({
                    url: "<%=request.getContextPath()%>/device/edit.html",
                    type: 'POST',
                    cache: false,
