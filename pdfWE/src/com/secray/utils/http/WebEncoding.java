...
public class WebEncoding {
    ...
    public static void main(String[] args) {
        ...
        try {
            System.out.println(web.getCharset("http://b2b.huangye88.com/"));
            // System.out.println(web.getCharset("http://www.163.com/"));
        } catch (IOException e) {
    ...
    public String getCharset(String strurl) throws IOException {
        ...
        urlConnection.setRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/51.0.2704.103 Safari/537.36");
        ...
        String htmlcode = sb.toString().replace(">\"","\"");
        // 解析html源码，取出<meta />区域，并取出charset

        String[] metas = RegUtil.getRegContent(htmlcode, RegUtil.META,2,"@");
        for(String meta :metas){
            if(meta.indexOf("charset")>-1){
                String[] charsets = meta.split(";");
                strencoding = (charsets.length == 2 ? charsets[1] : charsets[0]);
                return strencoding.split("=")[1];
            }
        }
